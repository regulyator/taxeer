...
func (q *Queries) GetRecordById(ctx context.Context, id uuid.UUID) (TaxeerRecord, error) {
...
const getRecordByUserIdWithLimitAndOffset = `-- name: GetRecordByUserIdWithLimitAndOffset :many
SELECT id, taxeer_user_id, date, income_value, income_currency, rate
FROM taxeer_record
WHERE taxeer_user_id = $1
ORDER BY date desc
...
type GetRecordByUserIdWithLimitAndOffsetParams struct {
	TaxeerUserID uuid.UUID
	Limit        int32
	Offset       int32
}

func (q *Queries) GetRecordByUserIdWithLimitAndOffset(ctx context.Context, arg GetRecordByUserIdWithLimitAndOffsetParams) ([]TaxeerRecord, error) {
	rows, err := q.db.QueryContext(ctx, getRecordByUserIdWithLimitAndOffset, arg.TaxeerUserID, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
